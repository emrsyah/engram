...
export function ShortcutsDialog() {
	...
	return (
		<Dialog open={shortcutsOpen} onOpenChange={(open) => !open && closeShortcuts()}>
			<DialogContent className="gap-0 overflow-hidden border-line-2 bg-[var(--color-panel)80] p-0 backdrop-blur-xl sm:max-w-[480px]">
				<DialogHeader className="border-line border-b px-6 pt-6 pb-4">
				...
				<div className="grid gap-x-8 gap-y-6 px-6 py-5 sm:grid-cols-2">
					{SECTIONS.map((section) => (
						<div key={section.title}>
							<p className="mb-3 font-bold text-ink-faint text-[11px] uppercase tracking-[0.12em]">
								{section.title}
							</p>
							<div className="space-y-2">
								{section.shortcuts.map((shortcut) => (
									<div key={shortcut.label} className="flex items-center justify-between gap-4">
										<span className="text-ink-3 text-sm">{shortcut.label}</span>
										<span className="flex shrink-0 items-center gap-1">
											{shortcut.keys.map((key) => (
												<kbd
													key={key}
													className="min-w-[22px] rounded-[4px] border border-line-strong bg-fill px-1.5 py-0.5 text-center font-mono text-ink-muted text-[11px]"
												>
													{key}
												</kbd>
											))}
