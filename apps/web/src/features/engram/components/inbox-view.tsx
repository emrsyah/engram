...
export function InboxView() {
  const { inboxItems, spaces, fileItem, removeItem, toggleDone } = useEngramStore();
    ...
    <section className="h-full overflow-y-auto bg-[#151310] px-8 py-10 text-white md:px-16 lg:px-28">
      <div className="mx-auto max-w-[860px]">
        <div className="flex items-end justify-between gap-5">
          <div>
            <h2 className="stagger-item flex items-center gap-3 font-bold text-3xl" style={{ animationDelay: "0ms" }}>
              <Icons.inbox className="size-7 text-[#9b88ff]" />
              Inbox
            </h2>
            <p className="stagger-item mt-3 max-w-2xl text-[#b0a69a]" style={{ animationDelay: "40ms" }}>
        ...
        <div className="mt-9 space-y-2.5">
          {inboxItems.length === 0 ? (
            <div className="stagger-item flex flex-col items-center gap-3 rounded-[10px] border border-dashed border-[#34302b] px-6 py-16 text-center" style={{ animationDelay: "80ms" }}>
...
function InboxRow({
  ...
  return (
    <div
      className={cn(
        "stagger-item group flex items-start gap-3 rounded-[10px] border border-[#2a2621] bg-[#1b1815] px-4 py-3",
        "transition-[border-color,background-color] duration-150 hover:border-[#3a352e] hover:bg-[#201d19]",
      )}
      style={{ animationDelay: `${80 + index * 30}ms` }}
    >
      {item.type === "task" ? (
        <span className="pt-0.5" onClick={(e) => e.stopPropagation()}>
          <Checkbox checked={item.done} onCheckedChange={onToggleDone} className="rounded-full" />
        </span>
      ) : (
        <span className="grid size-5 shrink-0 place-items-center pt-0.5">
          <span className={cn("size-2 rounded-[2px]", ACCENT_DOT[item.accent])} />
        </span>
      )}
      ...
      <button type="button" onClick={onOpen} className="min-w-0 flex-1 text-left">
        <div className="flex items-center gap-2">
          ...
          <span className={cn("truncate font-semibold text-[#f0ebe3]", item.done && "text-[#756e65] line-through")}>
