...
export function ItemNode({ data, selected }: NodeProps<ItemNodeType>) {
	const { item } = data;
	const { toggleDone, removeItem, connectItems, pinToFocus, unpinFromFocus } =
		useEngramStore();
	const { linkSourceId, setLinkSource, openDetail, openNoteEditor } =
		useUIStore();
	...
	return (
		<Card
			...
			onPointerUp={(e) => {
				...
				if (target.closest(".nodrag,button,a,input,textarea,[role='menuitem']"))
					return;
				if (!linkSourceId || linkSourceId === item.id) return;
				e.stopPropagation();
				finishLink();
		...
		>
			...
			{item.focusPinned && (
				<span
					className="pointer-events-none absolute top-0 left-0 z-10 flex size-5 items-center justify-center rounded-tl-[inherit] rounded-br-[6px] bg-[#907ce8]"
					title="In focus session"
				>
					<Icons.target className="size-3 text-[#17131f]" />
				</span>
			)}
				...
				{linkSourceId && linkSourceId !== item.id && (
					<button
						...
						className={cn(
							...
							"transition-[colors,transform] duration-100 active:scale-[0.92]",
						)}
					...
					<DropdownMenuContent side="bottom" align="end" sideOffset={6}>
						...
						{item.type === "task" && (
							<DropdownMenuItem
								onClick={() =>
									item.focusPinned
										? unpinFromFocus(item.id)
										: pinToFocus(item.id)
								}
							>
								<Icons.target className="size-3.5" />
								{item.focusPinned ? "Remove from focus" : "Add to focus"}
							</DropdownMenuItem>
						)}
						<DropdownMenuSeparator />
						<DropdownMenuItem
